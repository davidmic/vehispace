...
class EmailAndPasswordAuth extends ChangeNotifier {
 ...
 Future <FirebaseUser> getCurrentUser() async {
  ...
  Future<FirebaseAuth> signInUser({String email, String password, String userToken}) async {
    try {
     final  user = await _auth.signInWithEmailAndPassword(email: email, password: password);
     final userResult = user.user;
     if (userResult.isEmailVerified) {
         status = EStatus.authenticated;
       await _store.collection('usermanagement').document(userResult.uid).updateData({
         'userToken' : userToken,
       });
         notifyListeners();
     }else {
